package com.skillw.uisystem.internal.core
...
class ScriptGui(data: ScriptAnnotationData, player: Player): Gui() {
    val script = data.script
    val vars = script.script.engine.getBindings(ScriptContext.ENGINE_SCOPE)

    override val key = vars["key"]?.toString() ?: error("无法找到${script.key}的变量key")
    override val chest = vars["chest"]?.let { anyToBoolean(it) } ?: false
